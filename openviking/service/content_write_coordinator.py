...
class ContentWriteCoordinator:
    ...
    async def write(
        ...
        *,
        ...
        if context_type == "memory" and regenerate_semantics:
        ...
        if not regenerate_semantics:
            await self._write_in_place(normalized_uri, content, mode=mode, ctx=ctx)
            vector_updated = False
            queue_status = None
            if revectorize:
                await self._vectorize_single_file(
                    normalized_uri, context_type=context_type, ctx=ctx
                )
                vector_updated = True
                if wait:
                    queue_status = await self._wait_for_queues(timeout=timeout)
            return {
                "uri": normalized_uri,
                "root_uri": root_uri,
    ...
    async def _vectorize_single_file(
        ...
        *,
        ...
        await vectorize_file(
            file_path=uri,
            summary_dict={"name": os.path.basename(uri)},
